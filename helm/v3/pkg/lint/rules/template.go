...
import (
	...
	"agile-helm/helm/v3/pkg/lint/support"
)
...
func validateListAnnotations(yamlStruct *K8sYamlStruct, manifest string) error {
	if yamlStruct.Kind == "List" {
		...
		for _, i := range m.Items {
			if _, ok := i.Metadata.Annotations["agile-helm/resource-policy"]; ok {
				return errors.New("Annotation 'agile-helm/resource-policy' within List objects are ignored")
			}
		}
	}
	return nil
